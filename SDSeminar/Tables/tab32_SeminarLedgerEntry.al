table 50132 "CSD Seminar Ledger Entry"  //ledger entries
// Ledger tables contain the posted transactional information that is the prmary focus of its application area
// Ledger tables keep transaction history for the functional area
// This table is noneditable
// Also users cannot insert new entries directly into Ledger tables but only through posting routines that move data from journal tables to ledger tables
// After you post, the lines that were posted are deleted from the journal tables
{
    ...
    fields
    {
        field(1; "Entry No."; Integer)  //starts at 1 and its consecutive and unbroken
        {
            Caption = 'Entry No.';
        }
    ...
    keys
    {
        ...
        // Chapter 8 - Lab 2-1
        // Added key2 to enable Navigate to search for Seminar Management transactions in the most efficient way
        key(key2; "Document No.", "Posting Date")  //secondary keys are used for reports, pages and FlowFields
        {
        }
    }
}

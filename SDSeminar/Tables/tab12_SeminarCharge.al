table 50112 "CSD Seminar Charge"
{
    ...
    //   Chapter 6 - Lab 1
    //     - Created new table
    // Subsidiary table for the Seminar Registration Header table used for seminar expenses
    // It holds charges that are related to the seminar registration, in addition to the individual participant charges of the Seminar Registration Line table
    // This table uses the conditional relationship concept by relating to the G/L Account and Resource tables from the No. field based on the value specified in the Type field
    ...
    fields
    {
        ...
        field(3; Type; Option)
        {
            ...
            begin
                ...
                if Type <> xRec.Type then begin
                    //when the type field changes the record is initialized by calling the Init function
                    OldType := Type;  //the value of the type field must be stored before the Init function call
                    Init; //because init function initializes all the non-primary key fields that include the Type field
                    Type := OldType;    //then the value is retrieved
                end;
            end;
        }
        field(4; "No."; Code[20])
        {
            ...
            trigger OnValidate();
            //shows how the values of several fields are defaulted from other master records
            begin
                case Type of
                    Type::Resource:
                        begin
                            Resource.Get("No.");
                            Resource.TestField(Blocked, false);  //making sure that the record is not blocked
                    ...
                    Type::"G/L Account":
                        begin
                            GLAccount.Get("No.");
                            GLAccount.CheckGLAcc();  //does the same thing as the first TestField lines on the other Type
        ...
        field(6; Quantity; Decimal)
        {
            ...
            begin
                ...
                //If you do not want to manually check the Amount Rounding Precision field in the General Ledger Setup table, you can omit the rounding precision parameter when it calls the ROUND function.
            end;
        }
        field(7; "Unit Price"; Decimal)
        {
            ...
            trigger OnValidate();
            begin
                VALIDATE(Quantity); //"Total Price" := Round("Unit Price" * Quantity,0.01);
            end;
        }
        ...
        field(11; "Unit of Measure Code"; Code[10])  //influences the price calculations
        {
